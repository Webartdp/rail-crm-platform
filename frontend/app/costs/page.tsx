...
export default function CostsPage() {
  ...
  useEffect(() => {
    getWorkEventCosts()
      .then((response) => {
        ...
        setMessage(response.data.length === 0 ? 'No approved cost items yet.' : 'Loaded from API.');
      })
  ...
  return (
    <main className="page-shell">
      <RoleGuard allowedRoles={['manager', 'admin']} title="Costs access">
        <section className="hero-card">
          <div>
            <p className="eyebrow">Costs</p>
            <h1>Work Event Costs</h1>
            <p className="hero-text">Calculated from approved work and travel duration with employee tariff settings.</p>
          </div>
